import { Link } from 'react-router-dom'
import { showPreferences } from 'vanilla-cookieconsent'
import logo from '../assets/images/logo.svg'

export default function Footer() {
  return (
    <footer className="bg-dark text-white">
      <div className="max-w-content mx-auto px-6 pt-12 pb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 pb-10 border-b border-white/10">
          {/* Brand */}
          <div className="md:col-span-1">
            <img src={logo} alt="Green Heaven" className="h-8 mb-4 brightness-0 invert" />
            <p className="text-subtle text-sm leading-relaxed">
              Připojte se k nám a chraňte přírodu společně s námi!
            </p>
            <div className="flex gap-3 mt-5">
              <a href="https://www.instagram.com/greenheaven2011?igsh=MXE0bHk5NHJua285eA==" aria-label="Instagram" className="text-subtle hover:text-white transition-colors">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
              <a href="https://www.facebook.com/greenheaven.cz" aria-label="Facebook" className="text-subtle hover:text-white transition-colors">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Navigace */}
          <div>
            <h4 className="text-white font-semibold text-sm mb-4">Navigace</h4>
            <ul className="space-y-2.5">
              {[['O nás', '/o-nas'], ['Služby', '/sluzby'], ['Oblast působení', '/kde-pusobime'], ['Blog', '/blog'], ['Kontakt', '/kontakt']].map(([label, path]) => (
                <li key={path}>
                  <Link to={path} className="text-subtle text-sm hover:text-white transition-colors">
                    {label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Služby */}
          <div>
            <h4 className="text-white font-semibold text-sm mb-4">Služby</h4>
            <ul className="space-y-2.5">
              {['Výkup použitých olejů', 'Likvidace gastro odpadu', 'Biologické čištění odpadních vod', 'Nádoby na odpad'].map((s) => (
                <li key={s}>
                  <Link to="/sluzby" className="text-subtle text-sm hover:text-white transition-colors">
                    {s}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Kontakt */}
          <div>
            <h4 className="text-white font-semibold text-sm mb-4">Kontakt</h4>
            <ul className="space-y-2.5">
              <li className="text-subtle text-sm">+420 720 871 930</li>
              <li className="text-subtle text-sm">info@greenheaven.cz</li>
              <li className="text-subtle text-sm">GREEN HEAVEN s. r. o.</li>
              <li className="text-subtle text-sm">Hamr 34, 391 81 Val</li>
              <li className="text-subtle text-sm">IČO 28139160</li>
            </ul>
          </div>
        </div>

        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-6">
          <p className="text-subtle text-sm">© 2026 Green Heaven. Všechna práva vyhrazena.</p>
          <div className="flex gap-6">
            <button
              onClick={showPreferences}
              className="text-subtle text-sm hover:text-white transition-colors cursor-pointer"
            >
              Nastavení cookies
            </button>
          </div>
        </div>
      </div>
    </footer>
  )
}
